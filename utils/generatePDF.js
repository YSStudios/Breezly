...
const generatePDF = (formData) => {
  ...
  const content = `
    <div style="font-family: Times New Roman, serif; font-size: 12px; line-height: 1.5; padding: 40px 40px 20px 40px;">
      ...
      <p style="font-weight: bold; margin-bottom: 0;">${formData.buyerFirstName} ${formData.buyerLastName}</p>
      <p style="margin-top: 0; margin-bottom: 5px;">of ${formData.buyerAddress}</p>
      ...
      <p style="font-weight: bold; margin-bottom: 0;">${formData.sellerFirstName} ${formData.sellerLastName}</p>
      <p style="margin-top: 0; margin-bottom: 5px;">of ${formData.sellerAddress}</p>
      ...
      <p style="margin-bottom: 15px;">1. The Property is located at: ${formData.propertyAddress}. The land description is as follows: ${formData.propertyDescription}. All Property included within this Offer is referred to as the "Property".</p>
      ...
      <p style="margin-bottom: 15px;">${formData.includedItems || '_____________________________________________________________________________'}</p>

      <h3 style="font-size: 14px; margin-bottom: 10px;">Sales Price</h3>
      <p style="margin-bottom: 10px;">3. The total purchase price of $${formData.purchasePrice} (the "Purchase Price") that is to be paid for the Property by the Buyer is payable as follows:</p>
      <p style="margin-left: 20px; margin-bottom: 10px;">a. The initial earnest money deposit (the "Deposit") accompanying this offer is $${formData.earnestMoneyDeposit}. The Deposit will be paid by cash on or before ${formData.depositDate}. The Deposit will be held in escrow by ${formData.escrowAgent} until the sale is closed, at which time this money will be credited to the Buyer, or until this Offer is otherwise terminated; and</p>
      ...
      <p style="margin-bottom: 15px;">4. ${formData.escrowAgent} will return the Deposit to the Buyer if the Offer is rejected or expires prior to acceptance.</p>

      <h3 style="font-size: 14px; margin-bottom: 10px;">Closing & Possession</h3>
      <p style="margin-bottom: 15px;">5. The Closing Date will be on or be prior to ${formData.closingDate} or at such other time agreed by the Parties, at which point the Buyer will take possession of the Property.</p>
      ...
      ${Array.isArray(formData.conditions) ? formData.conditions.map((condition, index) => `<p style="margin-left: 20px; margin-bottom: 10px;">${String.fromCharCode(97 + index)}. ${condition}</p>`).join('') : ''}

      <h3 style="font-size: 14px; margin-bottom: 10px;">Additional Clauses</h3>
      <p style="margin-bottom: 15px;">7. ${formData.additionalClauses}</p>
      ...
      <p style="margin-left: 20px; margin-bottom: 0;">Name: ${formData.buyerFirstName} ${formData.buyerLastName}</p>
      <p style="margin-left: 20px; margin-bottom: 0;">Address: ${formData.buyerAddress}</p>
      ...
      <p style="margin-left: 20px; margin-bottom: 0;">Name: ${formData.sellerFirstName} ${formData.sellerLastName}</p>
      <p style="margin-left: 20px; margin-bottom: 0;">Address: ${formData.sellerAddress}</p>
      <p style="margin-left: 20px; margin-bottom: 0;">Phone: ___________________________</p>
      <p style="margin-left: 20px; margin-bottom: 15px;">Email: ___________________________</p>
      ...
      <p style="margin-bottom: 5px;">Seller's Name: ${formData.sellerFirstName} ${formData.sellerLastName}</p>
      <p style="margin-bottom: 5px;">Address: ${formData.sellerAddress}</p>
      <p style="margin-bottom: 5px;">Date: ____________________________</p>
